...
function main()
    result_root=abspath(ENV["DIRECTION_RESULT_ROOT"])
    passes=parse(Int,get(ENV,"DIRECTION_PASSES","4"))
    passes==4 || error("Production rolling driver requires four directions")
    ...
    try
        ...
        for pass in 1:passes
            ...
            if rank==root
                ...
                manifest=validate_checkpoint(current,L_num)
                get(manifest,"complete",false)==true || error("Incomplete direction checkpoint")
                open(joinpath(result_root,"directions.tsv"),"a") do io
                    println(io,pass,'\t',rev ? "reverse" : "forward",'\t',energy,
                        '\t',truncation,'\t',manifest["metadata"]["checkpoint_sha256"])
                end
                println("ROLLING_DIRECTION_RESULT pass=",pass,
                    " direction=",rev ? "reverse" : "forward",
                    " energy=",energy," truncation=",truncation,
        ...
        initialized && cleanup_environment_store!()
    end
    rank==root && println("four_direction_rolling=PASS passes=4")
    return nothing
end
